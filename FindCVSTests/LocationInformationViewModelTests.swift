...
class LocationInformationViewModelTests: XCTestCase {
    ...
    func testSetMapCenter() {
        ...
        //최초 현재 위치 이벤트
        let initailMapPoint = MTMapPoint(geoCoord:
            MTMapPointGeo(latitude: Double(37.394225), longitude: Double(127.110341)))!
        let currentLocationEvent = scheduler.createHotObservable([
            .next(0, initailMapPoint)
        ])
        ...
        expect(currentMapCenterObserver.events).to(
            equal([
                .next(0, initailMapPoint.mapPointGeo().latitude),
                .next(1, secondMapPoint.mapPointGeo().latitude),
                .next(2, initailMapPoint.mapPointGeo().latitude),
                .next(3, initailMapPoint.mapPointGeo().latitude),
            ])
        )
    }
}
